...
QuadraticManifoldMapping<TIn, TOut>::QuadraticManifoldMapping()
    : Parent()
    , d_quadraticManifoldBundle(initData(&d_quadraticManifoldBundle, "quadraticManifoldBundle",
        "Path to quadratic-manifold bundle directory. REQUIRED."))
{
    d_quadraticManifoldBundle.setPathType(sofa::core::objectmodel::PathType::DIRECTORY);
}

template <class TIn, class TOut>
void QuadraticManifoldMapping<TIn, TOut>::init()
{
    const unsigned n_in = this->fromModel->getSize();
    const unsigned n_out = this->toModel->getSize();

    m_projector = std::make_unique<sofa::component::kernel::QuadraticManifoldProjector>();
    m_projector->loadFromBundle(d_quadraticManifoldBundle.getValue());

    msg_info(this) << "loaded quadratic-manifold bundle "
                   << d_quadraticManifoldBundle.getValue()
...
template <class TIn, class TOut>
void QuadraticManifoldMapping<TIn, TOut>::ensureJ()
{
    if (!m_J_dirty) return;
...
template <class TIn, class TOut>
void QuadraticManifoldMapping<TIn, TOut>::reset()
{
    m_J_dirty = true;
    Parent::reset();
}
...
template <class TIn, class TOut>
void QuadraticManifoldMapping<TIn, TOut>::apply(const core::MechanicalParams* /*mparams*/,
                                             Data<VecCoord>& dOut,
                                             const Data<InVecCoord>& dIn)
{
    SCOPED_TIMER("apply in QuadraticManifoldMapping");
...
template <class TIn, class TOut>
void QuadraticManifoldMapping<TIn, TOut>::applyJ(const core::MechanicalParams* /*mparams*/,
                                              Data<VecDeriv>& dOut,
                                              const Data<InVecDeriv>& dIn)
{
    SCOPED_TIMER("applyJ in QuadraticManifoldMapping");
...
template <class TIn, class TOut>
void QuadraticManifoldMapping<TIn, TOut>::applyJT(const core::MechanicalParams* /*mparams*/,
                                               Data<InVecDeriv>& dOut,
                                               const Data<VecDeriv>& dIn)
{
    SCOPED_TIMER("applyJT in QuadraticManifoldMapping");
...
template <class TIn, class TOut>
void QuadraticManifoldMapping<TIn, TOut>::applyJT(const core::ConstraintParams* /*cparams*/,
                                               Data<InMatrixDeriv>& dOut,
                                               const Data<MatrixDeriv>& dIn)
{
    SCOPED_TIMER("applyJT(constraint) in QuadraticManifoldMapping");
