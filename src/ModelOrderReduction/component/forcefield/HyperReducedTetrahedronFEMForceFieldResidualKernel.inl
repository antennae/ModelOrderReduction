...
*   residual-kernel counterpart of HyperReducedTetrahedronFEMForceFieldKPCA.inl
******************************************************************************/
#pragma once

#include "sofa/core/behavior/BaseLocalForceFieldMatrix.h"
#include <ModelOrderReduction/component/forcefield/HyperReducedTetrahedronFEMForceFieldResidualKernel.h>
...
// scene was rewired by `modifyGraphSceneResidualKernel` between addObject and SOFA's
// init pass.
//
...
HyperReducedTetrahedronFEMForceFieldResidualKernel<DataTypes>::HyperReducedTetrahedronFEMForceFieldResidualKernel()
{
}


template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldResidualKernel<DataTypes>::accumulateForceSmall( Vector& f, const Vector & p, VecElement::const_iterator elementIt, Index elementIndex )
{
...
template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldResidualKernel<DataTypes>::applyStiffnessSmall( Vector& f, const Vector& x, int i, Index a, Index b, Index c, Index d, SReal fact )
{
...
template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldResidualKernel<DataTypes>::accumulateForceLarge( Vector& f, const Vector & p, VecElement::const_iterator elementIt, Index elementIndex )
{
    ...
    {
        msg_warning(this) << "TODO(HyperReducedTetrahedronFEMForceFieldResidualKernel): support for assembling system matrix when using plasticity.";
    }
}


template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldResidualKernel<DataTypes>::accumulateForcePolar( Vector& f, const Vector & p, VecElement::const_iterator elementIt, Index elementIndex )
{
...
template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldResidualKernel<DataTypes>::accumulateForceSVD( Vector& f, const Vector & p, VecElement::const_iterator elementIt, Index elementIndex )
{
...
template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldResidualKernel<DataTypes>::applyStiffnessCorotational( Vector& f, const Vector& x, int i, Index a, Index b, Index c, Index d, SReal fact )
{
    Displacement X;
...
template <class DataTypes>
void HyperReducedTetrahedronFEMForceFieldResidualKernel<DataTypes>::init()
{
    ...
    // at runtime ResidualKernelMapping::applyJT does the contraction, so m_qState is unused.
    ...
    if (this->d_prepareECSW.getValue())
    {
        ...
        if (!m_qState)
        {
            msg_error(this) << "residual-kernel force field requires a Vec1d MechanicalObject "
                               "of size " << expected << " (the ResidualKernelMapping's input "
...
template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldResidualKernel<DataTypes>::addForce(const core::MechanicalParams* /*mparams*/, DataVecDeriv& d_f, const DataVecCoord& d_x, const DataVecDeriv& /* d_v */)
{
    VecDeriv& f = *d_f.beginEdit();
    const VecCoord& p = d_x.getValue();

    SCOPED_TIMER("MORTetraResidualKernel::addForce");
    f.resize(p.size());
    ...
    // residual-kernel-specific: refresh the per-frame J(q_def) cache that updateGie consumes.
...
template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldResidualKernel<DataTypes>::addDForce(const core::MechanicalParams* mparams, DataVecDeriv& d_df, const DataVecDeriv& d_dx)
{
...
template<class DataTypes>
void HyperReducedTetrahedronFEMForceFieldResidualKernel<DataTypes>::draw(const core::visual::VisualParams* vparams)
{
...
template <class DataTypes>
void HyperReducedTetrahedronFEMForceFieldResidualKernel<DataTypes>::buildStiffnessMatrix(core::behavior::StiffnessMatrix* matrix)
{
