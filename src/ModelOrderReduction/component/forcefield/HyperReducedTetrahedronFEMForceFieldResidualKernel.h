...
*   residual-kernel counterpart of HyperReducedTetrahedronFEMForceFieldKPCA.
*   Same physics + ECSW handling; the only behavioral differences are
*   (a) inheritance from HyperReducedHelperResidualKernel, and (b) addForce calls
...
*   the residual-kernel helper's updateGie.
*
*   Runtime: option B (mapping-projects). The element loop writes
*   ξ_e · f_e to the Vec3d output and ResidualKernelMapping::applyJT contracts to q
...
class HyperReducedTetrahedronFEMForceFieldResidualKernel
    : public virtual TetrahedronFEMForceField<DataTypes>
    , public modelorderreduction::HyperReducedHelperResidualKernel
{
public:
    SOFA_CLASS2(SOFA_TEMPLATE(HyperReducedTetrahedronFEMForceFieldResidualKernel, DataTypes),
    ...
    // `modifyGraphSceneResidualKernel`, and to cross-parented Rigidify/articulated
...
public:
    ...
    HyperReducedTetrahedronFEMForceFieldResidualKernel();
...
extern template class SOFA_MODELORDERREDUCTION_API HyperReducedTetrahedronFEMForceFieldResidualKernel<defaulttype::Vec3Types>;
#endif
} // namespace sofa::component::forcefield
