...
*   see the matching .h for the rationale. Physics and the edge-based tangent
*   are byte-for-byte the linear-ROM sibling; the only addition is the
*   prepareFrame(u) call at the top of addForce.
******************************************************************************/
#pragma once
...
#include <ModelOrderReduction/component/forcefield/HyperReducedTetrahedronHyperelasticityFEMForceFieldKPCA.h>
...
template <class DataTypes> HyperReducedTetrahedronHyperelasticityFEMForceFieldKPCA<DataTypes>::HyperReducedTetrahedronHyperelasticityFEMForceFieldKPCA()
{
}
...
template <class DataTypes> void HyperReducedTetrahedronHyperelasticityFEMForceFieldKPCA<DataTypes>::init()
{
...
void HyperReducedTetrahedronHyperelasticityFEMForceFieldKPCA<DataTypes>::addForce(const core::MechanicalParams* /* mparams */ /* PARAMS FIRST */, DataVecDeriv& d_f, const DataVecCoord& d_x, const DataVecDeriv& /* d_v */)
{
    ...
    // as displacements in the bundle), built at bundle dim via m_indexMap.
    // Rigid-group verts (idx == -1) don't enter the kPCA displacement.
    // Identical wiring to the linear-tetra kPCA sibling.
    if (this->d_prepareECSW.getValue())
    {
        const auto& X0  = this->m_projector->X0();
        const auto& idx = this->m_indexMap;
        if (static_cast<long>(x.size()) != idx.size())
            throw std::runtime_error(
                "HyperReducedTetrahedronHyperelasticityFEMForceFieldKPCA: FF mstate has " +
                std::to_string(x.size()) + " verts but indexMap has " +
                std::to_string(idx.size()) + " entries.");
        Eigen::VectorXd u = Eigen::VectorXd::Zero(X0.size());
        for (unsigned vi = 0; vi < x.size(); ++vi)
        {
            const int def_i = idx(vi);
            if (def_i < 0) continue;
            for (unsigned c = 0; c < 3; ++c)
                u(3 * def_i + c) = x[vi][c] - X0(3 * def_i + c);
        }
        this->prepareFrame(u);
    }
...
template <class DataTypes>
void HyperReducedTetrahedronHyperelasticityFEMForceFieldKPCA<DataTypes>::updateTangentMatrix()
{
    unsigned int i=0,j=0,k=0,l=0;
...
template <class DataTypes>
void HyperReducedTetrahedronHyperelasticityFEMForceFieldKPCA<DataTypes>::addDForce(const core::MechanicalParams* mparams /* PARAMS FIRST */, DataVecDeriv& d_df, const DataVecDeriv& d_dx)
{
...
template <class DataTypes>
void HyperReducedTetrahedronHyperelasticityFEMForceFieldKPCA<DataTypes>::
...
template<class DataTypes>
void HyperReducedTetrahedronHyperelasticityFEMForceFieldKPCA<DataTypes>::draw(const core::visual::VisualParams* vparams)
{
