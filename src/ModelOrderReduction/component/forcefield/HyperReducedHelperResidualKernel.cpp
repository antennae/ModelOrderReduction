...
using sofa::component::kernel::ResidualKernelProjector;
...
HyperReducedHelperResidualKernel::HyperReducedHelperResidualKernel()
    ...
    , d_residualKernelBundle(initData(&d_residualKernelBundle, std::string("residual_kernel"),
          "residualKernelBundle",
    ...
    , d_indexMap(initData(&d_indexMap, "indexMap",
          "Per-mstate-vertex map into the residual-kernel bundle slot."))
{
    static const std::string groupName{"HyperReduction"};
    d_prepareECSW.setGroup(groupName);
    d_residualKernelBundle.setGroup(groupName);
...
void HyperReducedHelperResidualKernel::initMOR(unsigned nbElements, bool printLog)
{
    if (d_prepareECSW.getValue())
    {
        m_projector = std::make_unique<ResidualKernelProjector>();
        ...
        if (printLog)
            msg_info("HyperReducedHelperResidualKernel")
                << "loaded residual-kernel bundle " << d_residualKernelBundle.getValue()
        ...
        if (mapData.empty())
            throw std::runtime_error(
                "HyperReducedHelperResidualKernel: indexMap Data field is empty.");
    ...
    if (d_performECSW.getValue())
    {
        ...
        if (m_RIDsize == 0)
        {
            msg_warning("HyperReducedHelperResidualKernel")
...
void HyperReducedHelperResidualKernel::prepareFrame(const Eigen::Ref<const Eigen::VectorXd>& q)
{
    m_J = m_projector->J(q);
    m_frameReady = true;
}
...
void HyperReducedHelperResidualKernel::saveGieFile(unsigned nbElements)
{
    if (d_prepareECSW.getValue())
    {
