...
{
public:
    SOFA_CLASS(HyperReducedHelperResidualKernel, BaseObject);
...
template <class DataTypes>
Eigen::VectorXd HyperReducedHelperResidualKernel::projectOneElement(
    const std::vector<unsigned int>&              indexList,
    const std::vector<typename DataTypes::Deriv>& contrib) const
{
    ...
    for (unsigned i = 0; i < V; ++i)
    {
        if (static_cast<long>(indexList[i]) >= m_indexMap.size())
            throw std::runtime_error(
                "HyperReducedHelperResidualKernel: vertex index " +
...
template <class DataTypes>
void HyperReducedHelperResidualKernel::updateGie(
    const std::vector<unsigned int>&              indexList,
    const std::vector<typename DataTypes::Deriv>& contrib,
    unsigned int                                  numElem)
{
