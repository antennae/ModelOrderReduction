...
*   — see the matching .h for the rationale. Physics and the edge-based
*   tangent are byte-for-byte the linear-ROM sibling; the only additions are
*   the Vec1d-latent-state lookup in init() and the prepareFrame(q_def) call
*   at the top of addForce.
******************************************************************************/
#pragma once
...
#include <ModelOrderReduction/component/forcefield/HyperReducedTetrahedronHyperelasticityFEMForceFieldAE.h>
...
// none. See the AE linear-tetra sibling for why the size filter is
// essential on Rigidify + articulated scenes (servo-angle Vec1d states of
// the wrong size must be skipped).
...
template <class DataTypes> HyperReducedTetrahedronHyperelasticityFEMForceFieldAE<DataTypes>::HyperReducedTetrahedronHyperelasticityFEMForceFieldAE()
{
}
...
template <class DataTypes> void HyperReducedTetrahedronHyperelasticityFEMForceFieldAE<DataTypes>::init()
{
    ...
    // initMOR has just populated m_nbRigid/m_nbDef from the bundle.
...
void HyperReducedTetrahedronHyperelasticityFEMForceFieldAE<DataTypes>::addForce(const core::MechanicalParams* /* mparams */ /* PARAMS FIRST */, DataVecDeriv& d_f, const DataVecCoord& d_x, const DataVecDeriv& /* d_v */)
{
    ...
    // AE-specific: refresh the per-frame J(q_def) cache that updateGie
    // consumes. q is read from the cached Vec1d mstate pointer (parent chain).
    // Identical wiring to the linear-tetra AE sibling.
    if (this->d_prepareECSW.getValue() && m_qState)
    {
        ...
        else
        {
            ...
            for (Eigen::Index jq = 0; jq < nbDef; ++jq)
                q_def(jq) = q_pos[nbRigid + jq][0];
...
template <class DataTypes>
void HyperReducedTetrahedronHyperelasticityFEMForceFieldAE<DataTypes>::updateTangentMatrix()
{
    unsigned int i=0,j=0,k=0,l=0;
...
template <class DataTypes>
void HyperReducedTetrahedronHyperelasticityFEMForceFieldAE<DataTypes>::addDForce(const core::MechanicalParams* mparams /* PARAMS FIRST */, DataVecDeriv& d_df, const DataVecDeriv& d_dx)
{
...
template <class DataTypes>
void HyperReducedTetrahedronHyperelasticityFEMForceFieldAE<DataTypes>::
...
template<class DataTypes>
void HyperReducedTetrahedronHyperelasticityFEMForceFieldAE<DataTypes>::draw(const core::visual::VisualParams* vparams)
{
