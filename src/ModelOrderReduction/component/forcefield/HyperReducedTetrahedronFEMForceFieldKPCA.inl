...
*   kPCA counterpart of HyperReducedTetrahedronFEMForceField — see the
*   matching .h for the rationale.
******************************************************************************/
#pragma once

#include "sofa/core/behavior/BaseLocalForceFieldMatrix.h"
#include <ModelOrderReduction/component/forcefield/HyperReducedTetrahedronFEMForceFieldKPCA.h>
...
inline void HyperReducedTetrahedronFEMForceFieldKPCA<DataTypes>::accumulateForceSmall( Vector& f, const Vector & p, VecElement::const_iterator elementIt, Index elementIndex )
{
...
template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldKPCA<DataTypes>::applyStiffnessSmall( Vector& f, const Vector& x, int i, Index a, Index b, Index c, Index d, SReal fact )
{
...
template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldKPCA<DataTypes>::accumulateForceLarge( Vector& f, const Vector & p, VecElement::const_iterator elementIt, Index elementIndex )
{
    ...
    {
        msg_warning(this) << "TODO(HyperReducedTetrahedronFEMForceFieldKPCA): support for assembling system matrix when using plasticity.";
    }
}


template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldKPCA<DataTypes>::accumulateForcePolar( Vector& f, const Vector & p, VecElement::const_iterator elementIt, Index elementIndex )
{
...
template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldKPCA<DataTypes>::accumulateForceSVD( Vector& f, const Vector & p, VecElement::const_iterator elementIt, Index elementIndex )
{
...
template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldKPCA<DataTypes>::applyStiffnessCorotational( Vector& f, const Vector& x, int i, Index a, Index b, Index c, Index d, SReal fact )
{
    Displacement X;
...
template <class DataTypes>
void HyperReducedTetrahedronFEMForceFieldKPCA<DataTypes>::init()
{
...
template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldKPCA<DataTypes>::addForce(const core::MechanicalParams* /*mparams*/, DataVecDeriv& d_f, const DataVecCoord& d_x, const DataVecDeriv& /* d_v */)
{
    VecDeriv& f = *d_f.beginEdit();
    const VecCoord& p = d_x.getValue();

    SCOPED_TIMER("MORTetraKPCA::addForce");
    f.resize(p.size());
...
template<class DataTypes>
inline void HyperReducedTetrahedronFEMForceFieldKPCA<DataTypes>::addDForce(const core::MechanicalParams* mparams, DataVecDeriv& d_df, const DataVecDeriv& d_dx)
{
...
template<class DataTypes>
void HyperReducedTetrahedronFEMForceFieldKPCA<DataTypes>::draw(const core::visual::VisualParams* vparams)
{
    ...
    {
        const VecCoord& x2 = this->mstate->read(core::vec_id::read_access::position)->getValue();
        std::vector< type::Vec3 > points[3];
        for(unsigned ii = 0; ii<  x2.size() ; ii++)
        {
            Coord a = x2[ii];
            Transformation R;
...
template <class DataTypes>
void HyperReducedTetrahedronFEMForceFieldKPCA<DataTypes>::buildStiffnessMatrix(core::behavior::StiffnessMatrix* matrix)
{
